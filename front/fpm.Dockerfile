...
COPY ./composer.json ./composer.lock /var/www/vhosts/site/
RUN cd /var/www/vhosts/site && composer install --no-scripts --no-autoloader
...
RUN mkdir -p /var/www/vhosts/site/storage/framework/cache \
    && mkdir -p /var/www/vhosts/site/storage/framework/views \
    && mkdir -p /var/www/vhosts/site/storage/logs

RUN chown -R www-data:www-data /var/www/vhosts/site/storage
